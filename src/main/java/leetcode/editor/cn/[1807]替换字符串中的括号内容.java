...
// 
//
// Related Topics 数组 哈希表 字符串 👍 24 👎 0

// 替换字符串中的括号内容	题目标题	示例:两数之和
// 1807	题目编号
...
class EvaluateTheBracketPairsOfAString {
    ...
    class Solution {
        public String evaluate(String s, List<List<String>> knowledge) {
            ...
            // 构建字典
            Map<String, String> dic = new HashMap<>(knowledge.size());
            for (List<String> strings : knowledge) {
                dic.put(strings.get(0), strings.get(1));
            }
            // 记录(的位置,初始为-1
            int start = -1;
            ...
            for (int i = 0; i < chars.length; i++) {
                ...
                // s 中每对括号内的键都不会为空。
                // s 中不会有嵌套括号对。
                // 如果为) 那么必定已经有(在前面,需要截取中间的值为key
                if (c == ')') {
                    String key = s.substring(start + 1, i);
                    // 去字典内找key对应的value,如果找不到则设为?,并拼接到结果后面
                    str.append(dic.getOrDefault(key, "?"));
                    // 需要将(的位置设置为-1
                    start = -1;
                    continue;
                }
