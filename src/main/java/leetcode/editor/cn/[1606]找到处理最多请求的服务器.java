...
class FindServersThatHandledMostNumberOfRequests {
    ...
    class Solution {
        public List<Integer> busiestServers(int k, int[] arrival, int[] load) {
            ...
            int max = 0;
            int[] count = new int[k];
            PriorityQueue<Pair<Integer, Integer>> busy = new PriorityQueue<>((o1, o2) -> o1.getKey() - o2.getKey());
            for (int i = 0; i < arrival.length; i++) {
                // 每次进来都先将完成了的放入available中
                while (!busy.isEmpty() && busy.peek().getKey() <= arrival[i]) {
                    Pair<Integer, Integer> poll = busy.poll();
                    available.add(poll.getValue());
                }
                // 没有空闲的服务器,直接放弃任务
                ...
                count[server]++;
                max = Math.max(count[server], max);
                // 添加进busy list
                busy.add(new Pair<>(arrival[i] + load[i], server));
                // 从可用列表中移除
