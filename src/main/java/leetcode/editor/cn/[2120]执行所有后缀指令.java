...
class ExecutionOfAllSuffixInstructionsStayingInAGrid {
    public static void main(String[] args) {
        Solution solution = new ExecutionOfAllSuffixInstructionsStayingInAGrid().new Solution();
    }
    ...
    class Solution {
        public int[] executeInstructions(int n, int[] startPos, String s) {
            ...
            for (int i = 0; i < chars.length; i++) {
                ans[i] = executeInstructions(n, startPos, chars, i);
            }
            return ans;
        }

        public int executeInstructions(int n, int[] startPos, char[] chars, int start) {
            int ans = 0, x = startPos[0], y = startPos[1];
