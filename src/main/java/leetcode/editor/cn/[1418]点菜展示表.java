...
class DisplayTableOfFoodOrdersInARestaurant {
    public static void main(String[] args) {
        ...
        ans.add(Arrays.asList("David,3,Ceviche".split(",")));
        ans.add(Arrays.asList("Corina,10,Beef Burrito".split(",")));
        ans.add(Arrays.asList("David,3,Fried Chicken".split(",")));
        ans.add(Arrays.asList("Carla,5,Water".split(",")));
        ans.add(Arrays.asList("Carla,5,Ceviche".split(",")));
        ans.add(Arrays.asList("Rous,3,Ceviche".split(",")));
        List<List<String>> lists = solution.displayTable(ans);
        System.out.println("=========");
    ...
    class Solution {
        public List<List<String>> displayTable(List<List<String>> orders) {
            ...
            // 用来储存桌号信息 k是桌号,value是这一桌点了那些餐, value中的k是名称,value 是数量
            Map<Integer, Map<String, Integer>> count = new TreeMap<>();
            String table, temp;
            for (List<String> order : orders) {
                table = order.get(1);
                Map<String, Integer> orDefault = count.computeIfAbsent(Integer.parseInt(table), k -> new HashMap<>());
                temp = order.get(2);
                food.add(temp);
                Integer orDefault1 = orDefault.getOrDefault(temp, 0);
                orDefault.put(temp, orDefault1 + 1);
            }
            List<List<String>> ans = new ArrayList<>();
            ...
            food.forEach(k -> {
                indexMap.put(k, index.getAndIncrement());
            });
            count.forEach((k, v) -> {
                        String[] arr = new String[food.size() + 1];
                        Arrays.fill(arr, "0");
                        arr[0] = Integer.toString(k);
                        v.forEach((k1, v1) -> {
                            arr[indexMap.get(k1)] = Integer.toString(v1);
                        });
